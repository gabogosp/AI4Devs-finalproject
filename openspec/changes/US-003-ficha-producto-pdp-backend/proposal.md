...
|---|---|---|
| **AC-1** (ver ficha de publicado + URL amigable) | `GET /v1/products/{sku}` devuelve nombre, descripción, precio ARS (IVA incl.), imagen, categoría y disponibilidad de un producto **publicado** | **URL por slug → OQ-BE-1 `[Open]`**: `products` no tiene columna `slug` (sí `sku` e `id`). Interino: identificador público = `sku`. La columna `slug` es cambio de esquema **infra-owned**, escalado, NO materializado acá |
