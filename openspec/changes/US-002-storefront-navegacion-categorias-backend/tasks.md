...
- [ ] T1.1 `CategoriesRepository.findRoots()` y `findBySlugWithFamily(slug)`
...
- [ ] T2.1 `ProductsRepository.findPublishedByCategoryIds(ids, { limit, offset })`
...
- [ ] T4.1 `StorefrontService.getCategoryTree()` / `getCategoryBySlug(slug)` / `listPublishedProducts(slug, page)`
...
- [ ] T5.1 `StorefrontCategoriesController` (3 rutas) registrado en `StorefrontModule`
...
- [ ] T6.1 Extender `StorefrontCacheInterceptor` con TTL por endpoint vía decorador
...
- [ ] T7.1 Emitir `category.viewed` en cada detalle de categoría OK
...
- [ ] T9.1 Contratos draft de los 3 endpoints + lint
...
- [ ] T9.2 Actualizar el spec publicado del servicio + README
  ...
  - **Verify**: `npx @stoplight/spectral-cli lint apps/api/docs/api/openapi.yaml && grep -q "/v1/categories/{slug}/products" apps/api/docs/api/openapi.yaml && grep -q "categor" apps/api/README.md`

## Verification (suite-level)
- [ ] Unit + integration pasan: `pnpm --filter @dsm/api test` (Jest `--ci`; integration reutiliza el Postgres de docker-compose :55432 con esquema `@dsm/db` — desviación consciente documentada en `test/jest.setup.js`)
- [ ] E2E-nest pasan: `pnpm --filter @dsm/api test:e2e`
- [ ] Lint + typecheck limpios: `pnpm --filter @dsm/api lint && pnpm --filter @dsm/api typecheck`
- [ ] Contract lint pasa: `npx @stoplight/spectral-cli lint openspec/changes/US-002-storefront-navegacion-categorias-backend/contracts/openapi/*.yaml apps/api/docs/api/openapi.yaml`
