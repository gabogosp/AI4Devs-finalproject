...
El browse por rubro/subrubro es la **red de seguridad del descubrimiento** (cuando la búsqueda IA de US-004 no alcanza) y la vía principal de **SEO** del storefront (PRD §1.2/§1.4: que DSM "se la encuentre en Google"). US-003 abrió la superficie de lectura pública de `apps/api` con la ficha (`GET /v1/products/{sku}`); esta US completa la **navegación**: el árbol de categorías de dos niveles (rubro → subrubro, `categories.parent_id` — E2E §8) y el listado paginado de **productos publicados** por categoría, que las páginas SSR de categoría (FE-US-002) consumen para renderizar HTML indexable.
...
- **`StorefrontProductListItemDto`** (item de grilla, orientado a `ProductCard`): `sku` (identificador público interino para el enlace a la ficha US-003, per OQ-BE-1 heredada), `name`, `price_ars_cents` + `currency: 'ARS'` (IVA incluido, AC-3), `image_url` (nullable → placeholder FE), `in_stock` (derivado `stock > 0` — AC-5, sin exponer nivel de inventario). Sin campos de administración.
...
| **AC-3** (listado con nombre, precio ARS, imagen, disponibilidad, paginado, enlace a ficha) | `StorefrontProductListItemDto` expone `name`, `price_ars_cents`+`currency` (IVA incl.), `image_url` (nullable), `in_stock`; el listado está paginado; `sku` es el identificador para enlazar a `GET /v1/products/{sku}` (US-003) | Placeholder de imagen y el link renderizado son FE |
...
- **Columna `products.slug`** (URL amigable de ficha) → OQ-BE-1 de US-003, infra-owned, sigue diferida; el enlace del listado usa `sku`.
...
- **OQ-BE-1 (heredada de US-003) — URL de ficha por `slug` de producto.** `[Sigue Deferred — owner: Arquitecto/infra]` El enlace del listado a la ficha (AC-3) usa `sku` como identificador público interino. Sin impacto nuevo en este change: cuando se resuelva, el DTO del listado suma/renombra el campo del enlace.
