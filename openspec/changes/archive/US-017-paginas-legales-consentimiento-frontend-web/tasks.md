...
- [x] **P2 — Suite verde y build de producción verde en el `HEAD` de partida** *(verde 2026-08-22: 63 archivos / 353 tests; build OK)*
  ...
  - **Qué probó exactamente este verde, y qué NO** *(precisado 2026-08-23 a pedido de la sesión de backend, que encontró que un "build verde" local puede no valer para CI porque `apps/web/.env.local` está gitignoreado)*. Verificado: (a) el build **falló** sin la variable y **pasó** con ella pasada explícitamente en la línea de comando — o sea que el verde vino del artefacto y no de la configuración de la máquina; en el momento de correr P2 **no existía** `apps/web/.env.local` (apareció después). (b) Comparadas las claves de `.env.local` contra `.env.example`: **ninguna extra**. (c) En el schema zod de `apps/web/src/lib/env.ts`, **`API_INTERNAL_ORIGIN` es la única que lanza**; las cuatro `NEXT_PUBLIC_*` tienen `default`.
  - **Consecuencia que importa más allá de esta US**: que las `NEXT_PUBLIC_*` tengan default significa que su ausencia **no rompe el build — hornea placeholders en silencio**. Un pipeline sin `NEXT_PUBLIC_SITE_URL` construye verde y publica canonicals y `sitemap.xml` apuntando a `localhost`. Es exactamente el modo de falla que cubre **US-019 T2.3**, y este análisis confirma que esa task no es teórica: el default la vuelve invisible para cualquier gate que sólo mire si el build pasó.
