...
- **OQ-FE-1 — URL de la ficha (AC-1).** `[Resolved: 2026-08-16 — opción A]` Ruta interina `/productos/{sku}` (los sku son legibles, p.ej. `REF-001`); cuando OQ-BE-1 agregue `products.slug`, el segmento cambia de param y `/productos/{sku}` redirige 301 (SEO preservado, churn mínimo — espeja el trade-off del BE). Descartadas: (B) bloquear por slug (detenía el critical path FE); (C) slug sintético `{nombre}-{sku}` (duplica canónicas si el nombre cambia).
