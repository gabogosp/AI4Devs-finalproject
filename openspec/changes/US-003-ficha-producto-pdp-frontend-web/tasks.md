...
>
> **Estimación dual**: **6 h AI-asistido** / **~11 h tradicional** (US §7 presupuesta FE en 8–12 h tradicional; `story_points_ai_assisted: 2`, ×~0.45 per Peng 2023). Horas por task = AI-asistido.
>
> **Decisiones pendientes**: las tasks ejecutan las recomendaciones de OQ-FE-1 (ruta `/productos/{sku}`), OQ-FE-2 (CTA deshabilitado con seam), OQ-FE-4 (`revalidate: 60`) y OQ-FE-5 (evento `pdp_shown`). Si el usuario ratifica otra opción, se regenera la task afectada antes de ejecutar.
...
| AC-9 | Precio vigente (sin caché indefinida) | T2.1, T3.1 | in this change (`revalidate: 60` — OQ-FE-4) |

Cobertura no-AC del design.md (F51): cliente isomorfo → T1.2; codegen → T1.1; `remotePatterns` + env de imagen → T4.2; env `NEXT_PUBLIC_SITE_URL`/`NEXT_PUBLIC_WHATSAPP_PHONE` + README → T7.1; observabilidad → T5.1; a11y transversal → T6.1; skeleton/error boundary del segmento → T3.1.
...
- [ ] T1.2 Cliente HTTP isomorfo: `customFetch` ejecutable en Server Components sin romper la caché de datos de Next (0.5 h)
  - **Pattern**:
    ...
    // donde init puede traer { next: { revalidate: 60 } } o { cache: 'no-store' }
    ```
...
- [ ] T2.1 `storefrontService.getProductBySku` sobre el cliente generado + validación Zod + `AppError` (0.5 h)
  - **Pattern**:
    ...
    const res = await storefrontGetProduct(sku, { next: { revalidate: REVALIDATE_S } });
    return parseContract(storefrontProductSchema, res.data); // Zod GENERADO, en el borde
    ```
    — per `frontend-standards.md` §3.3 (service hand-written sobre cliente generado) + §11.3/§11.5 (AppError tipado, repositorio por feature); reutiliza `parseContract` de `src/lib/http/contract.ts`.
  - **Exit criterion**: el servicio devuelve el producto tipado y validado en 200; propaga `AppError` `notFound` en 404 y `server`/`network` en 5xx/red (sin filtrar el body crudo); no declara ninguna interfaz del contrato a mano; unit tests con los handlers MSW generados + overrides por test (404, 500) verdes.
...
- [ ] T3.1 `app/(storefront)/productos/[sku]/page.tsx` — Server Component con caché explícita + 404 real + boundaries del segmento (0.75 h)
  - **Pattern**:
    ...
    // caché de datos: next: { revalidate: 60 } en el fetch del servicio (AC-9, OQ-FE-4)
    ```
  ...
  - **Exit criterion**: la ruta renderiza server-side la ficha para un producto publicado; un 404 del contrato ejecuta `notFound()` y la respuesta HTTP es **status 404** con `not-found.tsx` accionable (mensaje + enlaces útiles, copy §10.2 — no un 200 vacío); existen `loading.tsx` (skeleton con la forma de la ficha) y `error.tsx` (reintento + reporte Sentry) en el segmento `(storefront)`; la caché del fetch está declarada explícitamente (`revalidate: 60`); component/integration tests (MSW) cubren éxito y propagación del 404.
  - **Verify**: `pnpm --filter @dsm/web test -- --run src/features/storefront && test -f "apps/web/app/(storefront)/productos/[sku]/page.tsx" && test -f "apps/web/app/(storefront)/productos/[sku]/not-found.tsx" && grep -q "revalidate" apps/web/src/features/storefront/storefrontService.ts && grep -rq "notFound()" "apps/web/app/(storefront)/productos/[sku]/page.tsx" && pnpm --filter @dsm/web build`
...
- [ ] T5.1 Evento cliente `pdp_shown` (sin PII) + tipado del catálogo de eventos (0.25 h)
  - **Pattern**:
    ...
    ```
    — per `observability-patterns` §9.5.2 (evento `{screen}_shown` por pantalla) y §3.3 (el `sku` va como propiedad de **analytics/evento**, nunca como dimensión de métrica — cardinalidad); sin PII (lectura anónima). Motivo: con ISR el BE subcuenta `product.viewed` (OQ-FE-5, recomendación A).
...
- [ ] T6.2 Smoke E2E Playwright — SSR indexable + 404 real (red de seguridad del dev; batería completa owned-by-QA) (0.75 h)
  - **Pattern**:
    ```ts
    ...
    ```
    — per `playwright-stability` (locators por rol, sin `waitForTimeout`, seed per-test vía API — `page.route` NO sirve acá: el fetch SSR es server-side) + `frontend-next-standards.md` §9 (E2E contra `next build && next start`). Nota: producto draft ≡ inexistente desde afuera (404 uniforme del BE), un solo caso negativo basta en el smoke; la matriz completa es de QA-US-003.
  - **Exit criterion**: el spec crea y publica un producto por la API admin (sku único), navega a la ficha y **asserta sobre el body de la respuesta del server** (no el DOM hidratado) que nombre, precio y JSON-LD están en el HTML; asserta status 404 (no 200) para un sku inexistente; corre verde contra build de producción con la API local.
