...
- **OQ-QA-4 — DEFECTO hallado por la E2E: la API no habilita CORS.** `[Resolved: 2026-07-26 — el backend habilitó CORS con allowlist explícita por env (change backend, Fase 10 T10.1), y la auditoría del hallazgo destapó que faltaban tambien los otros dos controles §7 mandatorios: rate limiting/lockout en la superficie de auth (T10.2) y security headers (T10.3). Con eso T3.1 (4/4) y T4.2 (4/4) pasan contra el stack vivo.]` `[Contexto original: bloquea T3.1 (E2E) y T4.2 (a11y) — la costura cross-origin (web:3200 → API:3000) recibe OPTIONS 404 sin headers Access-Control-*, así que el browser bloquea el login real y toda llamada autenticada del panel. Es un follow-up de BACKEND (`app.enableCors({ origin: <web>, credentials })` en `main.ts`/`configureApp`), no se parchea desde QA (un test rojo es un defecto a surface, no a tapar). Los specs de T3.1/T4.2 quedan autorados y verdes en cuanto CORS aterrice — owner: backend, revisit: al habilitar CORS.]` Hallazgo legítimo del Layer-3: los tests dev-owned (FE con MSW, BE con supertest, ambos same-origin) no podían detectarlo; la E2E cross-stack sí. TC-022 (redirect sin sesión, sin llamada a la API) pasa, confirmando que el harness E2E funciona.
