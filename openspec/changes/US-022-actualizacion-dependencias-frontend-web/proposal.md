...
**[Deferred — owner: usuario, motivo: `vitest` 2.1.8 → 3.2.6 es el único salto de major de esta US (2 critical, sólo dev). Si romper la config de tests actual (provider de coverage, compatibilidad con `msw@2.7.0`, `@testing-library/react@16`, setup de jsdom) es candidato legítimo a diferirse por AC-6 sin dejar expuesto al cliente — el riesgo real es "abrir un sitio malicioso con el servidor de UI de Vitest escuchando", un escenario de desarrollo, no de producción. Opciones para decidir: (a) diferir completo con exclusión nominal en `scripts/.audit-exclusions.json` (owner + fecha de revisión); (b) intentar el bump en una rama de spike aislada y medir el blast radius antes de decidir; (c) forzarlo igual asumiendo el trabajo de arreglar lo que rompa. No se decide en este plan — `tasks.md` deja la tarea correspondiente bloqueada, sin ejecutar, hasta que el usuario elija.]**
