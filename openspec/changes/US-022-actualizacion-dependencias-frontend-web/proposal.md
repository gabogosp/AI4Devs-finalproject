...
**[Deferred — owner: usuario, motivo: T4.1 (`@playwright/test` 1.49.1 → 1.55.1) rompe reproduciblemente `e2e/pdp-invalidation.spec.ts` (el único spec que prueba AC-9 — invalidación on-demand del precio tras editar desde el panel). Diagnóstico hecho el 2026-09-06:**
- Con `@playwright/test@1.55.1` + `next@15.5.21` (T2.1-T2.5 ya aplicados): el spec falla **3/3** corridas, siempre por el mismo motivo — el `expect.poll` (timeout 5000ms, intervals `[200,300,500,500,1000]`) agota sin que el HTML servido refleje el precio nuevo tras editar desde `/admin/productos/{id}`.
- Con `@playwright/test@1.49.1` (sin bump, mismo `next@15.5.21`, mismo backend, mismo stub): el spec pasa **1/1**. Se aisló el bump de playwright como única variable (via `git stash` del cambio de T4.1, reinstalando, y comparando ambas corridas con puertos E2E aislados — `E2E_PORT`/`API_STUB_PORT` no-default, para descartar colisión con el `next-server` de otro worktree paralelo en la misma máquina, que efectivamente existía y se confirmó no pertenecer a este workspace).
- El comentario ya existente en el propio spec (línea ~61-64) documenta que esta prueba **ya era conocida como ~33% flaky en paralelo** antes de este change, por una carrera entre la purga fire-and-forget de caché y el poll. La hipótesis más plausible es que Playwright 1.55.1 (Chromium 140 vs la build anterior) introduce una latencia de arranque/navegación marginalmente mayor que angosta aún más esa ventana de carrera, llevando el ~33% histórico a un 100% reproducible en este entorno — pero no se confirmó la causa raíz exacta dentro de Playwright (no se leyó su changelog en profundidad).
- **No se aplicó ningún fix** (ni se tocó el spec, ni sus timeouts/intervals, ni el bump de playwright) — per guardrail de "Verify falla → STOP, no auto-arreglar". El bump de T4.1 quedó revertido (`package.json`/`pnpm-lock.yaml` de vuelta a `@playwright/test@1.49.1`, working tree limpio) hasta que el usuario decida: (a) ajustar el timeout/intervals del poll en el propio spec (ya documentado como una prueba con carrera conocida, ensancharla parece razonable pero es una decisión de test, no mía), (b) diferir el bump de playwright con una exclusión nominal si aplica a algún advisory de audit, o (c) investigar más a fondo la causa en Playwright antes de decidir.
- T4.1 queda **sin marcar `[x]`** en `tasks.md`.]**

Sin más preguntas abiertas fuera de estas tres.
