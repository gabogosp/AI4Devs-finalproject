...
Superficie cubierta: `GET/PUT /me/reviews/{productId}`,
...
| # | Requisito | Origen |
|---|---|---|
| R-1 | Un cliente con una orden `delivered` que incluye el producto puede dejar una reseña (1-5 estrellas + comentario opcional) vía `PUT /me/reviews/{productId}`. | AC-1, AC-2 |
...
| R-5 | `GET /me/reviews/{productId}` (auth) devuelve `{eligible, review}` en una sola llamada: `eligible` para decidir si mostrar el control, `review` (la propia, exista o no) para pre-llenar el form de edición. | AC-6, AC-7 (soporte de UX) |
...
| R-7 | El autor de una reseña oculta la sigue viendo (con `hidden: true`) si consulta `GET /me/reviews/{productId}` — transparencia, no censura invisible. | AC-8 |
...
| # | Requisito |
|---|---|
| N-1 | Un cliente sin una orden `delivered` que incluya el producto recibe `403 dsm:reviews/not-eligible` al intentar `PUT /me/reviews/{productId}` — verificado SIEMPRE server-side, sin importar lo que envíe el body. | AC-6 |
...
| N-4 | Una reseña oculta (`hidden_at` seteado) nunca aparece en `GET /products/{slug}/reviews` ni cuenta en su agregado, sin importar quién pregunte (esa distinción es exclusiva de `GET /me/reviews/{productId}` con la sesión del autor). | AC-8 |
