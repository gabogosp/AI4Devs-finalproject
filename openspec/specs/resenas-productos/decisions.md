...
| `PUT`, no `POST`/`PATCH`, para `/me/reviews/{slug}`. | Semántica de upsert idempotente sobre un recurso identificado por URL — exactamente lo que `PUT` describe. Evita dos verbos (alta/edición) que en la práctica son la MISMA operación (`upsert` de Prisma sobre el `@@unique` compuesto). |
| `GET /me/reviews/{slug}` separado del `PUT`. | La FE necesita responder "¿elegible?" y "¿ya reseñé?" ANTES de renderizar el control (AC-6/AC-7 piden que el control ni siquiera aparezca) — una sola llamada resuelve ambas preguntas. La alternativa (inferir elegibilidad de un 403 al intentar el `PUT`) es un antipatrón de "probar y fallar". |
...
| `/me/reviews/{slug}` identifica el producto por **slug**, no por UUID (2026-09-07, fix post-mortem del contrato original). | El contrato archivado declaraba `{productId}` como UUID, pero `StorefrontProductDto` (US-002/US-003) excluye `id` a propósito — la ficha pública, el único caller real, nunca tuvo forma de resolver un UUID. `ReviewsService.getOwn`/`upsertOwn` resuelven `slug → product_id` internamente contra `ProductsRepository.findIdBySlug` (sin filtrar por `status`, mismo motivo que `findManyBySlugs` del carrito: un producto archivado sigue siendo reseñable). La FK real (`reviews.product_id`) no cambió. |
