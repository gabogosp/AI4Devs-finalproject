...
| **SUPERSEDED (2026-09-06, decisión del PO tras prueba visual)**: sin `?next=`, el login redirige al **home** (`/`), no a `/mi-cuenta`. | La decisión original de esta US mandaba a `/mi-cuenta` por default. El PO reconsideró tras probar el demo: el destino esperado post-login es el home. `?next=` explícito sigue respetándose igual (saneado, T0.6) — sólo cambia el default sin él. `LoginForm.tsx` ajustado; `sanitizeNext(null)` ya devolvía `/`, así que el fix simplifica el código (menos una rama) además de cambiar el destino. |
