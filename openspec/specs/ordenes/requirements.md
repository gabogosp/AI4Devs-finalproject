...
| R-4 | Una transición a `status=ready` invoca `NotificationPort.orderReadyForPickup`; la entrega real corre por `ResendNotificationAdapter` (US-011) — con `RESEND_API_KEY` ausente cae a `LoggingNotificationAdapter` (dev/test). | AC-4 |
