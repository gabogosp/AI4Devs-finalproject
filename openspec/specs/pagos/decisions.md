...
### Desde US-013 backend (archivada 2026-09-06)

| Id | Decisión | Fundamento |
|---|---|---|
| D17 | `POST /admin/orders/{id}/cancel` vive en `payments/` (`CancelOrderService`/`OrderCancellationController`), no en `orders/`, aunque comparte el prefijo `/v1/admin/orders`. | `payments/` ya importa `checkout` (`OrdersRepository`), `stock` y `orders` (`NOTIFICATION_PORT`, D15) — construirlo del lado de `orders/` obligaría a `orders` a importar `payments`, creando el ciclo `orders → payments → orders`. Mismo precedente real que `PaymentConfirmationController`. |
| D18 | Cero migración de Prisma — verificado leyendo las migraciones ya aplicadas (`orders_status_check` con los 6 estados, `payments_status_check` con `refund_pending`, `orders.cancelled_at` ya existente), no asumido. | `orders.status`/`orders.cancelled_at`/`payments.status` ya admitían todo lo que este change escribe desde US-008/US-010. |
| D19 | El reembolso reusa literal el patrón `refund_pending` + `POST /admin/payments/retry-refunds` de US-010 (D12/D16) — la llamada real a `MercadoPagoClient.refund()` corre FUERA de la transacción Prisma que hace la transición+reintegro+historial. | Ningún mecanismo de reembolso nuevo — si la llamada real falla, la fila queda `refund_pending`, elegible sin cambios por el job ya existente. Nunca una llamada externa dentro de un `$transaction` abierto (mismo criterio que D9). |
| D20 | `provider='manual'` se trata igual que `provider='simulated_dsm'` para el reembolso (no-op, marca `refunded` directo) — decisión de este change, no un AC literal. | Un pago manual (efectivo/transferencia, US-023) tampoco tiene un gateway que llamar; el reembolso físico ocurre fuera del sistema. Ratificación humana pendiente, no bloqueante (D-11 de `requirements.md`). |
| D21 | `CancelOrderResponse` (schema propio de `pagos`) declara sus propios campos (mismo shape que `AdminOrderDetail` de `ordenes` + `refund`) en vez de un `$ref` cruzado a `../ordenes/contracts/openapi.yaml`. | Mismo criterio que D13 (duplicación chica y deliberada) — evita acoplar dos raíces vivas por un `$ref`. En el código TypeScript sí se reusa `AdminOrderDetailDto.fromWithHistory` por composición (no un `$ref` de contrato, una importación de función). |

### Desde US-013 frontend-web (archivada 2026-09-06)

| Id | Decisión | Fundamento |
|---|---|---|
| D22 | `OrderCancelAction.tsx` reusa `ConfirmDialog` sin modificarlo, mismo componente que `ProductActions.archive`/`OrderAnonymizeAction`. | Ninguna variación de UX nueva justifica un tercer componente de confirmación destructiva — `design-system.md` §7.5 ya nombra "confirmación de cancelar orden" como uno de los dos usos canónicos del Modal. |
| D23 | Reconciliación DIRECTA desde `CancelOrderResponse` (self-contained, D21) — sin segundo `GET`, mismo patrón que `OrderStatusActions.onConfirmed`. | A diferencia de `OrderAnonymizeAction` (cuyo endpoint devuelve un shape parcial y sí refetchea), acá el contrato ya trae todo `AdminOrderDetail` + `refund` — la razón de la diferencia está en el contrato de cada endpoint, no en una preferencia de este change. |
| D24 (fix, PR #72) | El gate de visibilidad ("ofrecer una nueva cancelación") y el de "mostrar el resultado de la última acción" son dos condiciones DISTINTAS, no una sola. La primera versión de `OrderCancelAction` las colapsaba en un único `return null` guardado por `order.status`, y como cancelar deja la orden en `cancelled` (el mismo estado que dispara el gate), el mensaje de éxito quedaba seteado en estado pero nunca se pintaba — el componente se desmontaba en el mismo render que debía mostrarlo. | Encontrado por `QA-013-E2E-3` (cross-stack, el único layer que simula el re-render real del padre reconciliando `order.status`) — invisible para el test de componente aislado, que nunca re-renderiza con props actualizadas. Corregido separando ambas condiciones + test de regresión que reproduce el `rerender` real. |
