...
| `GEMINI_MAX_RPM` | 5 | Cuota del **lote**, que reparte los 15 RPM del free tier con la búsqueda (`GEMINI_SEARCH_MAX_RPM=10`). El adapter espacia las salidas a `60000 / GEMINI_MAX_RPM`. **La suma se valida al arrancar**: subir uno sin bajar el otro no arranca. Para la primera corrida del catálogo se le da la cuota entera con dos overrides temporales (`15` + búsqueda en `0`) — runbook §3.6 |
