import { backoffDelayMs, MercadoPagoPermanentError, MercadoPagoTransientError, withRetry } from './backoff';

/**
 * T3.1 — reintentos. **Ningún test duerme**: el `sleep` se inyecta y sólo
 * acumula el tiempo simulado, así que la suite entera corre en milisegundos
 * de reloj real aunque simule minutos de espera.
 */
...
describe('backoffDelayMs', () => {
  ...
  it('crece exponencialmente: 750ms, 1.5s, 3s, 6s (con jitter fijo al 75%)', () => {
...
describe('withRetry', () => {
  ...
  it('un transitorio persistente ⇒ 4 invocaciones (1 + 3 reintentos)', async () => {
    ...
    const fn = jest.fn().mockRejectedValue(new MercadoPagoTransientError('503'));

    await expect(withRetry(fn, base(reloj))).rejects.toBeInstanceOf(MercadoPagoTransientError);
  ...
  it('un transitorio que después funciona ⇒ devuelve el resultado', async () => {
    ...
    const fn = jest
      .fn()
      .mockRejectedValueOnce(new MercadoPagoTransientError('429'))
  ...
  it('retryAfterSeconds de MercadoPago GANA sobre el backoff calculado', async () => {
    const reloj = relojSimulado();
    const fn = jest
      .fn()
      .mockRejectedValueOnce(new MercadoPagoTransientError('429', 30))
      .mockResolvedValue('ok');
  ...
  it('un MercadoPagoPermanentError NO se reintenta: 1 sola invocación', async () => {
    const reloj = relojSimulado();
    const fn = jest.fn().mockRejectedValue(new MercadoPagoPermanentError('400'));

    await expect(withRetry(fn, base(reloj))).rejects.toBeInstanceOf(MercadoPagoPermanentError);

    expect(fn).toHaveBeenCalledTimes(1);
    expect(reloj.esperas).toHaveLength(0);
  });

  it('un error que no es de MercadoPago tampoco se reintenta', async () => {
    const reloj = relojSimulado();
