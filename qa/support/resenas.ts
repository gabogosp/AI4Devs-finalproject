 ...
 * `PUT /v1/me/reviews/:productId` (upsert), `GET /v1/me/reviews/:productId`
...
export async function dejarReseña(
  ...
  const res = await ctx.put(`/v1/me/reviews/${productId}`, {
    data,
...
export async function miReseña(ctx: APIRequestContext, productId: string): Promise<RespuestaReseña> {
  const res = await ctx.get(`/v1/me/reviews/${productId}`);
  const body = await res.json().catch(() => undefined);
  return { status: res.status(), body };
}
